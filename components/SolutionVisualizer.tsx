...
import PackingSimulation from './PackingSimulation';
...
const SolutionVisualizer: React.FC<SolutionVisualizerProps> = ({ items, result, onReset }) => {
    ...
    const displayCapacity = (isTableTooLarge && !showFullTable) ? 20 : capacity;
    ...
    return (
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
            ...
            <PackingSimulation items={items} result={result} />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col justify-center gap-2">
                    <button
                        onClick={onReset}
                        className="flex items-center justify-center gap-2 text-sm text-slate-400 hover:text-white transition-colors p-1 rounded hover:bg-slate-800 h-full w-full"
                    >
                    ...
                    <div className="flex justify-between items-end mb-4">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                                Row $i$ = item considered. Col $w$ = capacity limit.
                            </p>
                        </div>
                        {isTableTooLarge && (
                            <button
                                onClick={() => setShowFullTable(!showFullTable)}
                                className="text-xs text-amber-500 flex items-center gap-1 bg-amber-900/20 px-2 py-1 rounded hover:bg-amber-900/30 transition-colors cursor-pointer"
                            >
                                <Info className="w-3 h-3" />
                                {showFullTable ? "Show Truncated View" : "Truncated View"}
                            </button>
                        )}
                    </div>
